Feature: Deletes company exemption resource from the database
  ...
  Scenario Outline: Successfully deletes a company exemptions resource from Mongo
    ...
    And a "<resource>" exists for the given "<company_number>"
    ...
    And the resource does not exist in Mongo for "<company_number>"

    Examples:
    | company_number | resource                      |
  ...
  Scenario: A DELETE request is sent without ERIC headers causing an unauthorised error
